...
DECLARE @failed BIT, @before_count INT, @after_count INT, @before_item_count INT, @after_item_count INT, @order_no VARCHAR(40), @items_json NVARCHAR(MAX);
...
SET @failed = 0; SET @order_no = CONCAT('AT-BADJSON-', LEFT(CONVERT(VARCHAR(36), NEWID()), 12));
BEGIN TRY
...
SET @failed = 0; SET @order_no = CONCAT('AT-ZERO-', LEFT(CONVERT(VARCHAR(36), NEWID()), 12));
...
SET @failed = 0; SET @order_no = CONCAT('AT-INACTIVE-', LEFT(CONVERT(VARCHAR(36), NEWID()), 12));
...
SET @failed = 0; SET @order_no = CONCAT('AT-NOBOM-', LEFT(CONVERT(VARCHAR(36), NEWID()), 12));
...
BEGIN TRY
BEGIN TRANSACTION;
DECLARE @customer_id_2 BIGINT, @product_id_2 BIGINT, @order_id_2 BIGINT, @failed_2 BIT, @order_no_2 VARCHAR(40), @items_json_2 NVARCHAR(MAX);
...
BEGIN TRY
BEGIN TRANSACTION;
DECLARE @customer_id_3 BIGINT, @product_id_3 BIGINT, @order_id_3 BIGINT, @amount_3 DECIMAL(10,2), @mismatch_amount_3 DECIMAL(10,2), @failed_3 BIT, @order_no_3 VARCHAR(40), @items_json_3 NVARCHAR(MAX), @mismatch_txn_3 VARCHAR(50), @success_txn_3 VARCHAR(50), @duplicate_txn_3 VARCHAR(50);
...
END;');
DECLARE @customer_id_4 BIGINT, @product_id_4 BIGINT, @order_id_4 BIGINT, @amount_4 DECIMAL(10,2), @failed_4 BIT, @order_no_4 VARCHAR(40), @items_json_4 NVARCHAR(MAX), @fault_txn_4 VARCHAR(50);
...
BEGIN TRY
BEGIN TRANSACTION;
DECLARE @customer_id_5 BIGINT, @product_id_5 BIGINT, @order_id_5 BIGINT, @amount_5 DECIMAL(10,2), @price_before DECIMAL(10,2), @price_after DECIMAL(10,2), @failed_5 BIT, @order_no_5 VARCHAR(40), @items_json_5 NVARCHAR(MAX), @snapshot_txn_5 VARCHAR(50);
